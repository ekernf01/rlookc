...
test_that("Diagnostic catches heteroskedasticity", {
  ...
  knockoffs = rlookc::computeGaussianKnockoffs(X, mu = 0, Sigma = cor(X), num_realizations = 19)
  calibration_regular = rlookc::simulateY(X, knockoffs, FUN = function(x) x)
  calibration_nayusty = rlookc::simulateY(X, knockoffs, FUN = "adversarial", kmeans_centers = 2)
  calibration_diverse = rlookc::simulateY(X, knockoffs, FUN = "diverse")
  expect_lt(
