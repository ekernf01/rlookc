...
# We start by generating a bimodal synthetic dataset, along with some good knockoffs and some mis-specified knockoffs that ignore bimodality.
# We'll show some calibration tools to try to detect the problem with the bad knockoffs.
set.seed(0)
...
# Using calibrate__simulateY, you can simulate from P(Y|X) and (since the active set is known) check any set of knockoffs you generate.
...
# You can use rlookc::calibrate__findWorstY to try to search more thoroughly for a worst-case P(Y|X).
...
# The search is over a pre-specified collection of realizations for Y. You can generate a default with calibrate__chooseDiverseY.                                                  
...
plot(X[,2], collection_of_y$y[[6]], main = "Apparently not as bad")
...
# For this example, we generate new X, new good knockoffs, and new bad knockoffs based on independent permutations.
set.seed(0)
X = rnorm(1e4) %>% matrix(ncol = 10)
...
# You can use the KNN test to pinpoint outliers. (This trick can have weak signal, though.)
# Let's make a few outrageous outliers in the last 50 observations and try to spot them. 
X[1:50,1:5] = -100
rlookc::calibrate__KNNTest(X, good_knockoffs, n_neighbors = 100)$prop_not_swapped_per_observation %>%
  head(100) %>%
  plot
# One pragmatic option is to generate knockoffs only for non-outlying observations.
# For the outliers, you can generate knockoffs that are exact dupes of the data.
# This will preserve the necessary exchangeability, possibly with a loss of power.
