...
calibration_check = rlookc::calibrate.simulateY(X, good_knockoffs)
calibration_check2 = rlookc::calibrate.simulateY(X, bad_knockoffs)
calibration_check3 = rlookc::calibrate.simulateY(X, bad_knockoffs, FUN = function(x) 2*x)
...
# You can use calibrate.chooseDiverseY and rlookc::calibrate.findWorstY to try
# to search more thoroughly for a worst-case P(Y|X).
# rlookc::calibrate.findWorstY will split the data, searching for the worst P(Y|X)
# in one half and checking calibration in the other half.
collection_of_y = rlookc::calibrate.chooseDiverseY(X, n_quantiles = 5)
worst_y = rlookc::calibrate.findWorstY(X, good_knockoffs,
                                       y = collection_of_y$y,
