---
title: "Generating knockoffs"
output:
  html_fragment: default
---

This vignette demonstrates the new features of our software that
allow construction of knockoffs in various ways that either have not been described
or have been described without open-source code:
...
You can generate leave-one-out knockoffs fast. This code created a list of matrices where element 6 contains knockoffs for X[,-6].
...
a set of low-rank updates to correct them as if they had been done with specified variables left out.
...
You can reconstitute the compact LOOKs in a more immediately usable form.
...
It is minimal but it is unit-tested. As input, it takes the files saved by `saveLoad.saveCompactLooks`.)
...
Here's a quick check of the knockoff exchangeability condition. Red is the original data and blue and green each have a different variable swapped with its knockoffs.
